...
void Chat::setNonBlockingTerminal() {
    ...
    string Chat::readLineNonBlocking() 
    {
    ...
    std::string Chat::repairUtf8(const std::string& input) {
    std::string repaired;
...
bool Chat::isValidUtf8Char(const unsigned char* bytes, size_t len) {
...
// 验证整个字符串是否为有效的 UTF-8
bool Chat::isValidUtf8(const std::string& str) {
    ...
    void Chat::startChatSession(const string& friendName) 
    {
...
//      }
// }
    void Chat::privateChat(int sockfd,std::string current_user)
    {   running=true;
        sock=sockfd;
    ...
    int Chat::isValidFriend(const std::string& friendName) 
    {
    ...
    void Chat::displayUnreadMessagesFromFriend(const string& friendName)
    {
    ...
    void Chat::queryChatHistory(const string& friendName)
    {
        json request = {
    ...
    void Chat::processChatHistory(const json& response) 
    {
        if (!response.value("success", false)) {
            cout << "无历史聊天记录" << endl;
            return;
