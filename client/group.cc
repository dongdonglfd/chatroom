#include"group.h"
    ...
    void Group::groupMenu(int sockfd, std::string current_user) 
    {
        sock = sockfd;
    ...
    bool Group::is_number(const std::string& s) {
    ...
    void Group::processJoinRequests(int groupId) 
    {
        // 获取待处理的加入请求
        json reqPending;
    ...
    // 添加群管理员 (群主权限)
    void Group::addGroupAdmin(int groupId) 
    {
    ...
    // 移除群管理员 (群主权限)
    void Group::removeGroupAdmin(int groupId) 
    {
    ...
    // 移除群成员 (群主/管理员权限)
    void Group::removeGroupMember(int groupId) 
    {
