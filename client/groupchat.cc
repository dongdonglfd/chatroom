...
void groupchat::sendLength(int fd, const json& response) {
 ...
 json groupchat::sendRequest(const json& request) {
    ...
    void groupchat::groupChat(int fd,string name)
    {
        userName=name;
        sock=fd;
        // 启动接收线程
    ...
    // 发送消息给指定用户
    void groupchat::sendMessage(const string& recipient, const string& text) 
    {
    ...
    string groupchat::readLineNonBlocking() 
    {
    ...
    void groupchat:: startgroupChat(int id)
    {
    ...
    void groupchat::receivegroupMessages()
    {
        char buffer[4096];
    ...
    void groupchat::displayUnreadMessagesFromgroup(string username)
    {
        json request = {
    ...
    void groupchat::processgroupHistory(const json& response) 
    {
