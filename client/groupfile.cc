...
json groupfile::sendRequest(int sock, const json& request) {
    ...
    void groupfile::sendgroupfile(int sockfd,string currentuser)
    {
        sock=sockfd;
    ...
    void groupfile::uploadgroupFileToServer(const std::string& filePath, int sock) 
    {
        string fileName = fs::path(filePath).filename().string();
    ...
    void groupfile::getUndeliveredgroupFiles(int client_sock, const std::string& username)
    {
        sock=client_sock;
    ...
    void groupfile::receivegroupfile(FileInfoo &selectedFile)
    {   
        {
