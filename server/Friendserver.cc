...
//     int data_sock=-1;
//     int listen_sock=-1;
Connection* Friendserver::getDBConnection() {
    try {
    ...
    void Friendserver::handleAddFriend(int fd, const json& req) 
    {
    ...
    // 处理查看请求列表
    void Friendserver:: handleCheckRequests(int fd, const json& req) 
    {
    ...
    // 处理请求操作
    void Friendserver::handleProcessRequest(int fd, const json& req) 
    {
        string requester = req["from"];
        string target = req["to"];
    ...
    //展示好友列表
    void Friendserver::handleGetFriends(int fd, const json& req)
    {
        string user = req["user"];
        unique_ptr<sql::Connection> con(getDBConnection());
        unique_ptr<sql::PreparedStatement> stmt(
    ...
    void Friendserver::handleDeleteFriend(int fd, const json& req)
    {
        string user = req["user"];
    ...
    void Friendserver::handleBlockUser(int fd, const json& req)
    {
        string user = req["user"];
    ...
    void Friendserver::handledisBlockUser(int fd, const json& req)
    {
        string user = req["user"];
