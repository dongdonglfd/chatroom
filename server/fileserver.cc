...
uint16_t pasv()
    ...
    void FileTransferServer::sendResponse(int fd, const json& response) {
        string responseStr = response.dump();
        send(fd, responseStr.c_str(), responseStr.size(), 0);
    }
    Connection* FileTransferServer::getDBConnection() {
        try {
    ...
    void FileTransferServer::handleFileUploadRequest(int fd, const json& request)
    {   
    ...
    void FileTransferServer::storeFileInfo(const string& sender, const string& recipient, 
    ...
    void FileTransferServer::handleFileStart(json& data, int client_sock) 
    {
    ...
    void FileTransferServer::handleFileEnd(json& data, int client_sock)
    {
        json response = {
            {"success", true}
        };
    ...
    void FileTransferServer::getUndeliveredFiles(int fd,json& data)
    {
    ...
    void FileTransferServer::privatefilesend(json& data, int client_sock)
    {
    ...
    void FileTransferServer::updateFileDeliveryStatus(json& data, int client_sock)
    {
        int id=data["fileid"];
        unique_ptr<sql::Connection> con(getDBConnection());
         // 检查数据库连接
