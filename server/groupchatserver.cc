...
json groupchat::sendRequest(const json& request) {
    ...
    void groupchat::handleGroupMessage(int client_sock, const json& data)
    {   
        sock=client_sock;
    ...
    void groupchat::broadcastToGroup(int group_id, const json& message, const string& excludeUser)
    {   
    ...
    void groupchat::storeOfflineMessage(const string& recipient, int group_id, const json& message)
    {
        cout<<"store"<<endl;
        unique_ptr<sql::Connection> con(getDBConnection());
    ...
    void groupchat::checkgroupUnreadMessages(int fd, const json& req)
    {
cout<<"123"<<endl;
        // 确保请求包含用户字段
    ...
    void groupchat::handleGetGroupUnreadMessages(int client_sock, const json& data)
    {
    ...
    void groupchat::handleGetGroupHistory(int client_sock, const json& data)
    {
        // 验证请求参数
        if (!data.contains("user") || !data["user"].is_string() ||
            !data.contains("groupid") || !data["groupid"].is_number()) {
