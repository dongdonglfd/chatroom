...
void groupfileserver::sendResponse(int fd, const json& response) {
        string responseStr = response.dump();
        send(fd, responseStr.c_str(), responseStr.size(), 0);
    }
    void groupfileserver::handleFilegroupUploadRequest(int fd, const json& request)
    {   
    ...
    void groupfileserver::broadcastToGroup(int fd,int group_id, const json& message, const string& excludeUser )
    {
        unique_ptr<sql::Connection> con(fileserver.getDBConnection());
        // 准备查询群组成员的SQL语句
    ...
    void groupfileserver::storeOfflinefile(const string& recipient, int group_id, const json& message,int fd)
    {
        unique_ptr<sql::Connection> con(fileserver.getDBConnection());
    ...
    void groupfileserver::handlegroupFileStart(json& data, int client_sock) 
    {
    ...
    void groupfileserver::handlegroupFileEnd(json& data, int client_sock)
    {
        json response = {
            {"success", true}
        };
    ...
    void groupfileserver::getUndeliveredgroupFiles(int fd,json& data)
    {
    ...
    void groupfileserver::updategroupFileDeliveryStatus(json& data, int client_sock)
    {
        int id=data["fileid"];
        unique_ptr<sql::Connection> con(fileserver.getDBConnection());
         // 检查数据库连接
    ...
    void groupfileserver::groupfilesend(json& data, int client_sock)
    {
